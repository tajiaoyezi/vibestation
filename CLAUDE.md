...
- **SPIKE-07.5 spec 已 ready（2026-05-16）**：路径 A 新 R1 gate · spec 详化（#340）+ ready-gate APPROVE-WITH-NITS 3 nit 修（#341）+ Arbiter flip 拍板（"批准 flip" · 翻转 gate 选项 a）· **实施 gated on Arbiter 自定执行窗口**（重录 36 结构化 corpus 消耗 API 预算 · 同 SPIKE-06 录制模式 · 不自动启动）
...
1. **SPIKE-07.5 实跑**（新 R1 gate · 路径 A · spec 已 `ready` @ #341 · **实跑 gated on Arbiter 自定执行窗口** · 不自动启动）：用 `claude -p --output-format stream-json --include-hook-events` + `codex exec` 重录结构化 corpus（非交互 · 非 TUI）→ 复用 SPIKE-07 ir.rs+assertions.rs+matrix 聚合（新写 .jsonl loader + 结构化 parser · 见 SPIKE-07.5 spec §B）→ 重跑 §F 矩阵 → 重走 SPIKE-07 §H 三路径。**MVP-18/19/20 实施的新唯一前置**（SPIKE-07 已 deferred · SPIKE-07.5 翻盘 R1 真降级才解锁 · 消耗 API 预算同 SPIKE-06）
