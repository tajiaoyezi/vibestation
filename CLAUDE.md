<!--
  ...
  阶段状态 / 文件导航 / 元信息在 docs/PROGRESS.md 和 docs/internal/SESSION-STARTUP.md。
...
**人类详细手册 + Playbook + FAQ**：`docs/internal/SESSION-STARTUP.md`（不在本文件重复）。
...
| 8   | 平台 = **macOS 15 + Ubuntu 24 + Windows 11（x64 MSVC）**，三平台并列当前 active scope（Windows 从 v0.4 提前 · ADR-024）                                                                                                                                                                                                                                                                                                                                   | [ADR-024](./docs/adr/ADR-024-windows-platform-pull-forward.md) · `implementation-plan.md` §1.4 / §2 / §3.1                                                                                                  |
...
| 18  | Runtime 证据路径 = **`docs/runtime-evidence/<task-id>/`**（MVP / feature · 进 git · Spike 走独立 4 样齐全归档）                                                                                                                                                                                                                                                                       | [ADR-011](./docs/adr/ADR-011-runtime-evidence-location.md) · [ADR-023](./docs/adr/ADR-023-capture-mandate-removed.md)（capture mandate removed）                                                              |
...
## 🏁 当前可执行动作（session 33-34 · 2026-05-17/20 · **MVP-18 Phase A/B/C + MVP-19 W1/W2/CDE-impl + 治理 ADR-021/022 + MVP-20 Phase A/C/D + 05-20 dependabot #396-#399 + PRE-CAPTURE-READINESS #401-#403 + ADR-023 capture mandate removed #405-#409 · 15 spec flip done · v1.0 vision 代码侧完整 ship 收口**）

> 权威当前态以 [`docs/PROGRESS.md`](./docs/PROGRESS.md) 为准（本 🏁 段下方 session-32 详述为历史快照 · 不再逐项追平 · 见「自审四问」边界：scalar 当前态归 PROGRESS）。**session 33 实况**：MVP-20 Phase A 全链 #385-#388 + Phase C #391/#392/#390 + **Phase D 全 merged #394**（主 agent 自实施 TDD：Part1 `RollbackStatusKind` union 保真〔spec §K · 裸 String→typed enum · 编译期抓越界 magic〕+ Part2 全局 `detect_crash_recovery` + 启动 emit + `lib/rollback-recovery` 状态机 + `RollbackRecoveryBanner` + App.tsx wire〔镜像 MVP-16 · session 维度平行〕+ Part3 abort 边界 · self-review v2-D.2 + Arbiter approve · §2.14 runtime smoke · build.rs 补 rollback_ops rerun 修 Phase A 遗留）· **2026-05-20 dependabot 4 PR housekeeping 全 merged**（A 顺序 #396 cargo patch `fa908b4` → #398 playwright minor `7c30826` → #397 npm patch group `a90f892` → #399 **shiki 3.23.0→4.1.0 major** `ad1097e` · admin direct merge per ADR-016 v2-D.2 豁免 trailer · 本地 4 worktree gate 全绿〔cargo 0 · vitest 59/497 ×4〕+ main on-merge 4.1.0 re-verify 全绿 · dev smoke 第 1 轮 exit 0 无 panic · 第 2 轮因 pnpm `npm_config_prefix` env 泄漏 user terminal abort 改信 vitest）· **2026-05-20 PRE-CAPTURE-READINESS 9 文件 3 PR 批量补全**（镜像 MVP-19 #384 模式 · 10/10 deferred MVPs 全覆盖）：PR-A #401 v1.0 vision MVP-18+20 `30f982c` · PR-B #402 v0.1 老 deferred MVP-04+05+09+10 `a941580` · PR-C #403 v0.2/v0.3 MVP-13+17 + v0.3 sprint batch（MVP-12/14/15/16）`be4a8fe` · 各 readiness 三分类（✅已验证/⚠️gap/🔴Arbiter本人）让 Arbiter 实跑前先读知 code-side 已 green 的部分不必重复验 · 9 新文件 + 0 改代码 + 0 改 spec · **2026-05-20 ADR-023 capture mandate removed**（supersede ADR-011 · Arbiter 命令"截图和录视频等任务全部删掉"）：PR-1 #405 spec MVP-04/05/06/08/09 capture 段删 `1f7ef7d` · PR-2 #406 spec MVP-10/12/13/14/15 `f961a3a` · PR-3 #407 spec MVP-16/17/18/19/20 `28cabfb` · PR-4 #408 删 21 治理文档（rule + 2 playbook + 10 PRE-CAPTURE-READINESS + 3 CAPTURE-PLAYBOOK + 3 metrics + CAPTURE-GUIDE + 2 capture script）+ cross-ref 改 `37bfb63` · PR-5 ADR-023 + 15 spec frontmatter ready/in-progress → done + PROGRESS/CLAUDE/tasks/README sync · **15 spec status 全翻 done**（含 MVP-18/19/20 v1.0 vision）· 现 spec done 计数 17 → **32**（仅余 SPIKE-06 blocked + template draft）· 已捕证据全保留 · 全局 rule 15 不动 · **v1.0 vision 代码侧 ship 完整收口** · **下一步 = 营销发布物料 / Apple Dev Program / 域名 TLD 等非代码项**（runtime 证据 + Criterion 性能量化 · defer Arbiter playbook #390 窗口 · spec 保持 `in-progress`：多 phase）· 追踪：🟠 DiffLine shiki pre-existing flaky（4.1.0 此次全过仍 transient）· Phase E capture 待 Arbiter playbook #390 窗口。
...
1. **MVP-18 Phase C · failure feedback wire**（关键路径 · 现已解锁 · #345 parser_bridge 已 main）：`ParserBridgeError → PaneLinkError::Parser*` IPC 边界 map + `pane:trigger`/`pane:build-failed` 事件 + `pane:failure:preview_prompt` 命令 + child failure pipeline。spec §I 估 ~5d · 不自动启动 · Arbiter 明确方向后认领。parser 实施复用 SPIKE-07.5 已证 sound 的 CliEvent IR + 结构化 adapter 架构（归档级原型 · 生产重写见 spec §C Don't.5）。Phase C done 后 MVP-19→20 依赖链解锁
   ...
   1c. **#329 dispatch-rule 压缩 · ✅ 已 resolved+merged**（review-gate BLOCK stale base 拦截 → Arbiter "a" 授权 → rebased onto main 重做压缩 · 47d2436 §2.9/§2.10 规范核心 7/7 保留非回退 · `.claude/rules/dispatch-prompt-template.md` 883→597 行 < 40k · 审计抽 `docs/internal/dispatch-incidents.md`）· **后续 dispatch 规则查审计详例改读 `docs/internal/dispatch-incidents.md`（稳定 `<a id>` 锚点 · 不进 auto-load）**
