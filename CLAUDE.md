...
4. **version bump + release（P1）**：当前 1.1.1 · 近 session 新功能（Output Tab / Windows titlebar / i18n）值得发版。先定是 v0.1.x 系列还是升 v0.2。
5. **可选代码改进（P2 · 不阻塞）**：
   - **SPIKE-07.6（可选 · 非 greenlight 阻断）**：codex 非退化 auth/network corpus 需真 OpenAI API key → 补强 codex 错误事件解析准确率。
   - **Windows app-menu 快捷键 gap**（#452 遗留）：Ctrl+T/W/, 在 Windows 缺失（Arbiter defer）。小修，可补。
   - **deferred capture playbook**（ADR-023 后非 ship 阻塞）：按 Arbiter 自定窗口跑 v0.3 sprint 的 GUI/metrics/visual/a11y capture。

详细阶段 / 进度 / 卡点见 [`docs/PROGRESS.md`](./docs/PROGRESS.md)；任务索引与 v1.0 spec 行数见 [`docs/tasks/README.md`](./docs/tasks/README.md)。

---

> **历史快照（session 32/33 实况 · 保留作审计 · 其「下一步候选」已被上方 supersede）**：以下记录 v1.0 vision 从 ready-gate 到实施全 done 的过程，不再作为行动依据。
