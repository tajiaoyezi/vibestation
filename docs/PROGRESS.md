...
| **Next concrete action**  | **session 20 进行中 · 8 PR merged · 主 agent 主线收尾阶段** · 已合：(a) PR #152 ADR-015 accepted · (b) PR #153 session-19.md 归档 · (c) PR #154 PROGRESS sync (D2) · (d) PR #155 MVP-10 Phase B Sentry SDK 主体（4 commits）· (e) PR #156 MVP-09 §D Criterion bench + §E 集成测试 · (f) PR #157 ADR README 索引同步（含 round 2 字节级 self-fix）· (g) PR #158 MVP-10 Phase B follow-up（§C.4 endpoint UI + §G.4 H2 proof + §F capture guide + §2.13 dispatch 规则 + clippy fix）· (h) PR #159 MVP-09 Phase C 错误流补强（19 dialog/toast CSS class 全填 + Hook stderr Copy 按钮 + exit code 显示）· **剩余 GUI 截图任务**（CLI 不能跑 · 等 Arbiter 本地 30-60 min 集中采集）：(1) MVP-05 Phase D runtime capture（`scripts/capture/mvp-05/capture-phase-d.sh` + `measure-memory.sh` 就位）· (2) MVP-10 §F 4 张截图（`docs/runtime-evidence/mvp-10/CAPTURE-GUIDE.md` 步骤完整）· (3) MVP-04 §I 22 张截图 + 2 录屏（cargo test 7 PASS / 15 ignore-runtime 已就位 · 仅缺截图）· **off-mainline**：MVP-04 §I shell tail-end · MVP-08 Phase E v0.2 deferred（A.6/F.3 fixture script 已就位）· SPIKE-06 §B Apple Dev 申请（用户决策中）· MVP-09 Phase C/D spec frontmatter 翻 done | session end   |
...
**阶段**：session 20 进行中 · 8 PR merged · 主 agent 主线收尾阶段 · session 19 已 36 PR 收官（PR #117-#152 · 详细见 session-19.md）· `MVP-04` Phase D shell 兼容 done · `MVP-05` Phase A/B/C 全 done（Phase B 4 layout pure functions + 17 tests · pane_pty 5 IPC · 5 layout IPC + transactional pane_service · 13 tests · Phase C scaffolding + 完成版 PR #144/#147-#151）· Phase D runtime capture 待 Arbiter 30 min · `MVP-08` Phase A/B/C/D 全 done · Phase E partial done + R-PHASE-E v0.2 deferred（A.6/F.3 fixture script 已就位）· `MVP-09` Phase A done · Phase B Status 面板 + CommitBar done（PR #118）· §D Criterion bench + §E 集成测试 done（PR #156）· **Phase C 错误流 UX 补强 done（PR #159 · 19 dialog/toast CSS class 全填 + Hook stderr Copy 按钮 + exit code · 修补隐藏 critical UX 缺口）** · `MVP-10` Phase A done · Phase B Sentry Spike done（PR #120）· **ADR-015 accepted by Arbiter（PR #152）· Phase B SDK 编码 done（PR #155 · 4 commits · 19 测试全过）· §C.4 endpoint UI + §G.4 H2 proof + §F capture guide + §2.13 dispatch rule done（PR #158）** · `MVP-11` Native Feel Quality 全 done · 5/5 phase（macOS + Linux）· 15 ADR（含 ADR-015 accepted）· v2-D.1 规则稳态运行
...
剩余 GUI 截图任务（CLI 不能跑 · 等 Arbiter 本地集中采集）:

- MVP-05 Phase D `metrics-mvp-05.md` 实测 + `01-04*.png` 截图（capture script 就位）
- MVP-10 §F 4 张截图（`01-settings-panel` / `02-settings-realtime` / `03-telemetry-opt-in` / `04-telemetry-decline` · CAPTURE-GUIDE.md 步骤完整）
- MVP-04 §I 22 张截图 + 2 段 30s 录屏（shell + CLI 实机矩阵）
...
| 🟡 MVP v0.1 进度            | **3/10 done + 7/10 ready**（MVP-02/03/07 done · MVP-01/04/05/06/08/09/10 ready · MVP-11 Native Feel done）· session 20 末 phase 级别全收敛：MVP-04 Phase A-F 全 done 仅 §I 截图待补 · MVP-05 Phase A/B/C 全 done · Phase D 待 GUI capture · MVP-08 Phase A-D 全 done · Phase E v0.2 deferred · MVP-09 Phase A/B/C/D 全 done（Phase D done by PR #156）· MVP-10 Phase A/B 全 done（B4-B5 done · §C.4/§G.4/§F follow-up done by PR #158）· **主线收敛到 GUI 截图采集 + spec frontmatter done 翻转 + 打包发布**（不再有需要新写代码的主线 task） |
