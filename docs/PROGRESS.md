...
| **Next concrete action**  | **session 20 启动 · 4 项遗留 + 业务推进** · session 19 已 36 PR 收官（详见 session-19.md）· session 20 入口已合 PR #152（ADR-015 accepted）+ PR #153（session-19.md 归档）· **下一步**：(1) MVP-05 Phase D runtime 实测 capture（Arbiter 本地 ~30 min · `scripts/capture/mvp-05/capture-phase-d.sh` + `measure-memory.sh` 就位 · `metrics-mvp-05.md` 待填实测 · v0.1 GA gate 前推荐）· (2) PROGRESS.md 本次同步（D2 任务 · Ubuntu agent · 当前进行中）· (3) MVP-09 剩余 Phase B 后续（Ubuntu Claude Code · 业务推进）· (4) MVP-10 Phase B Sentry SDK 编码（主 agent · ADR-015 已 accepted 解锁 · 估 1 session）· 主线外：MVP-04 §I 22 用例 follow-up · SPIKE-06 §B Apple Dev 申请（用户决策中） | session end   |
...
**阶段**：session 20 启动 · session 19 已 36 PR 收官（PR #117-#152 · 详细见 session-19.md）· `MVP-04` Phase D shell 兼容 done · `MVP-05` Phase A/B/C 全 done（Phase B 4 layout pure functions + 17 tests · pane_pty 5 IPC · 5 layout IPC + transactional pane_service · 13 tests · Phase C scaffolding + 完成版 PR #144/#147-#151）· Phase D runtime capture 待 Arbiter 30 min · `MVP-08` Phase A/B/C/D 全 done · Phase E partial done + R-PHASE-E 技术债（manual capture 待 Arbiter 或推 v0.2）· `MVP-09` Phase A done · Phase B Status 面板 + CommitBar done（PR #118）· 剩余 Phase B 推进中（B4 任务 · Ubuntu Claude Code）· `MVP-10` Phase A done · Phase B Sentry Spike done（PR #120）· **ADR-015 accepted by Arbiter（PR #152 · 2026-04-26）· 解锁 SDK 编码（B5 任务 · 主 agent · session 20 接）** · `MVP-11` Native Feel Quality 全 done · 5/5 phase（macOS + Linux）· 15 ADR（含 ADR-015 accepted）· v2-D.1 规则稳态运行
