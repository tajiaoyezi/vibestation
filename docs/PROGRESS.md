...
- 治理对齐 PR（session 33 · 本 PR）：ADR-021（CI mandate → 质量门 · 方案 b）+ ADR-022（dispatch 范本去断链 · 方案 d · 原 Context 经主 agent git 证伪）proposed → accepted · 同步改 CLAUDE.md §5 + dispatch-prompt-template §3.0/§4 + dispatch-incidents §4 + ADR-README（status/统计/治理类清单）· Arbiter tajiaoyezi 2026-05-17 拍板两方案
