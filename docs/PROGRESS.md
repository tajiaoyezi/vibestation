...
| **Next concrete action**  | **session 19 收尾 · PR #117-#145 共 29 PR 全合入** · **MVP-11 Native Feel Quality 5/5 Phase ✅ 全 done** · **MVP-05 Phase A/B 全 done + Phase C scaffolding done**（PR #141-#144 · pane_init IPC + 3 SolidJS 组件 + 快捷键 hook） · **ADR-006 Ubuntu validated · v0.1 GA 双平台解锁**（PR #138 · 双平台 X11 108ms / Wayland 107ms / 30 stable） · **Branch protection 机械化**（PR #145 · `.githooks/pre-push` + pnpm prepare 自动配置 · 每台机器 clone + pnpm install 即激活） · **本地分支全 cleanup**（18 origin/ → 1 · 仅 main）· **下一步**：(1) MVP-05 Phase C 完成版（Terminal.tsx 集成 PaneSplitView · 拖拽 splitter · Smart Layouts · §F 6 P99 + Phase D 5 截图 · 估 2-3h · 派 sub-agent 或主 agent 下次接） · (2) ADR-015 Arbiter approve 后主 agent 接 MVP-10 Phase B Sentry SDK 编码（团队 3 人 = 主 agent + 本机 Kimi + kimi-ubuntu24） · (3) 余下 manual capture：R-PHASE-E 3 DevTools P99（主 agent 顺手 · 0.5d）· 主线外 · MVP-04 §I 22 用例 follow-up · SPIKE-06 §B Apple Dev 申请（用户决策中） | session end   |
...
**阶段**：🎉 **session 19 收尾 · 29 PR merged · MVP-11 5/5 ✅ + MVP-05 Phase A/B/C-scaffolding ✅ + Branch protect 机械化 ✅ + ADR-006 Ubuntu validated**（2026-04-25 · session 19 已合入 PR #117-#145 共 29 PR · 史上最高产 · 本地分支全 cleanup · 18 origin/ branches → 1）· `MVP-04` Phase D shell 兼容 done · `MVP-05` Phase A storage prep done · **Phase B 全 done**（PR #141-#143 · 4 layout pure functions + 17 tests · pane_pty 5 IPC · 5 layout IPC + transactional pane_service · 13 tests · 文件域物理隔离 · §H.3 atomicity）· **Phase C scaffolding done**（PR #144 · pane_init_for_tab IPC + 3 SolidJS 组件 + 快捷键 hook · 0 集成 · 接 PR 待派）· `MVP-08` Phase A/B/C/D 全 done · Phase E 🟡 fix-up + R-PHASE-E 技术债（OpenCode hallucinate 4 数字 · 主 agent round 2 降为 partial done · v0.2 fixture generator 进 git PR #140）· `MVP-09` Phase A done · Phase B Status 面板 + CommitBar done · `MVP-10` Phase A 设置面板 done · Phase B Sentry Spike done（PR #120 · ADR-015 proposed 待 Arbiter approve · 解锁 SDK 编码）· `MVP-11` Native Feel Quality **全 done** · 5/5 phase（PR #119/#125 spec · #122-#131 五 phase · #139 Linux fallback 由 kimi-ubuntu24 补 · A.5 Linux + R-PHASE-2.linux 全 done） · CI billing failure 应对：workflow_dispatch only（PR #121）· **PR #137-#138 SPIKE-01+02 Phase B Ubuntu 验证 · ADR-006 caveat removed · v0.1 GA 双平台正式可发布** · **PR #145 branch protection 机械化**（`.githooks/pre-push` + pnpm prepare 自动配置 · 每台机器 clone + pnpm install 即激活 · 详见 CLAUDE.md L131）· 15 ADR（含 ADR-015 proposed）· v2-D.1 规则稳态运行
**日期**：2026-04-25（session 19 收尾 · 已合入 #117-#145 共 29 PR · 史上最高产 · MVP-11 5/5 + MVP-05 Phase A/B/C-scaffolding ✅ · 团队 3 人 = 主 agent + 本机 Kimi + kimi-ubuntu24）
**GitHub**：<https://github.com/tajiaoyezi/vibestation>（PRIVATE）
**已合入的 PR（滚动窗口 · 只保留最近 2 session · 更早见 `git log --all` + `docs/session-history/`）**：

### Session 19（2026-04-25 · MVP-11 Native Feel + ADR-015 Sentry + CI billing 修复 + MVP-05 Phase B/C scaffolding + Branch protect 机械化 · **29 PR merge · 史上最高产**）

- **PR #145 `4372104`**（**chore(hooks): pre-push hook 阻止直推 main · pnpm install 自动配置**）· Claude Code 主 agent · `.githooks/pre-push` 38 行 · 读 stdin ref 列表 · 阻 `refs/heads/main` push · `SKIP_BRANCH_PROTECT=1` Arbiter override · `package.json` 加 `prepare` script (`git config core.hooksPath .githooks`) · npm 标准 lifecycle 触发 · 无 husky 依赖 · CLAUDE.md L131 文档化机械防护 · 4 场景实测全过（main 阻 / bypass 通 / feat 通 / pnpm install 自动配）· 防其他 agent / 主 agent 误推 main
- **PR #144 `21418f4`**（**MVP-05 Phase C UI scaffolding · pane_init IPC + 3 SolidJS 组件 + 快捷键 hook**）· Claude Code 主 agent · backend `apply_pane_init_for_tab` + `PaneInitRequest` ts-rs binding（idempotent · 已初始化 tab 直返当前 PaneListResponse · 2 unit tests）· `PaneTerminal.tsx` 203 行（pane_pty_* 5 IPC + 2 events · WebGL→Canvas→DOM fallback）· `PaneSplitView.tsx` 131 行（递归 LayoutNode 渲染 · Single→PaneTerminal · Split→子 SplitView+Splitter）· `PaneSplitter.tsx` 36 行（1px 分隔条 · 双击复位 · 拖拽留 PR 完成版）· `usePaneShortcuts.ts` 89 行（⌘\ ⌘⇧\ ⌘⌃W · macOS keymap · capture phase keydown · shouldSuppress hook）· styles.css 加 vs-pane-* CSS · permission + capability 加 allow-pane-init-for-tab · **不修改 Terminal.tsx**（0 集成 · 留 Phase C 完成版 PR 接 sub-agent 或下次 session）· 288 tests pass（+2 新增）
- **PR #143 `20f8adf`**（**MVP-05 Phase B Step 2 IPC · 5 layout commands + transactional pane_service**）· Claude Code 主 agent · `crates/core/src/pane_service.rs` 500 行 · 5 transactional 函数（apply_pane_split / close / focus / layout_preset / split_ratio_update）· 调 PR #141 4 pure functions + rusqlite Transaction（panes 表 + tabs.layout + focused_pane_id 原子写 · §H.3 atomicity）· 任意一步 fail → 全部 rollback · 13 unit tests 覆盖 happy path + 错误路径 + atomicity rollback · `crates/app/src/lib.rs` 5 thin IPC wrapper + `generate_handler!` 注册 · permission + capability 加 5 allow-pane-* · 286 tests pass（+13）
- **PR #142 `771e66b`**（**MVP-05 Phase B Step 1 · pane_pty 5 IPC backend (§H.6 锁 A 独立命名空间)**）· Claude Code 主 agent · `crates/core/src/pane_pty.rs` 128 行 · 5 wrapper functions（spawn / stdin / resize / signal / kill）把 PanePty* IPC payload 转 Pty* · 用独立 PtyManager 实例 · 避免与 tab_pty collision · `AppState.pane_pty` 独立实例 · `emit_pane_pty_events` 线程 · 反向映射 PtyEvent → PanePtyEvent（pane_id 取代 tab_id）· `crates/app/permissions/pane-pty.toml` 5 permission · 273 tests pass（+3）
- **PR #141 `0155814`**（**MVP-05 Phase B Step 2 layout · 4 pure function + 17 单元测试 + 7 micro-bench**）· Sub-agent D（Claude Code · 后台 isolation worktree）· `crates/core/src/panes.rs` +647 行 · 4 pub pure function（split_layout / close_pane_in_layout / update_split_ratio / apply_smart_layout）· 不 mutate 入参 · 失败必 Err · validate_mvp_05() 守门 · 新 enum SmartLayoutKind { Solo / AiAndRunner } · `crates/core/benches/pane_layout_bench.rs` 138 行 7 bench 全 sub-microsecond（48-210 ns · 远 < 1ms 期望）· 17 tests（§H.2 6 + §H.3.1 atomicity 6 + Smart Layout 3 + ratio 2）· 273 → 270 tests / 0 fail
- **PR #140 `a0064d7`**（**chore(MVP-08): R-PHASE-E v0.2 fixture generator 脚本**）· Sub-agent C（Claude Code · 后台 isolation worktree）· `scripts/fixtures/mvp-08/gen-10k-diff.sh` 152 行 + `gen-1k-files.sh` 144 行 · README 6.5 KB + 反模式（中文全宽圆括号 U+FF08 触发 bash unbound var · ASCII 替换）· `set -euo pipefail` · 路径必填 / 不允许已存在 / 拒绝受保护路径 · self-test PASS · 让 R-PHASE-E A.6/F.3 真实数字测量可复现 · 闭环 R-PHASE-E 技术债
- **PR #139 `ef7c044`**（**feat(U3-U6): Ubuntu installer + MVP-11 Linux vibrancy/titlebar 验证**）· kimi-ubuntu24（远程独立电脑）· `crates/app/tauri.conf.json` bundle targets 加 deb · AppImage 78MB / deb 5.5MB build + 安装 + 卸载验证 · `docs/runtime-evidence/mvp-01/` 2 PNG · `docs/runtime-evidence/mvp-11/` 3 PNG（A.5 Light/Dark + B.4 GNOME default title bar）· **round 3 fix-up**（B4 真实捕获 · md5 与 A5 dark 不同 · scrot -u → import -window mutter-frame 根因修正）· MVP-01 spec §F 双平台打包 acceptance 全勾 · v0.1 GA macOS-first → 双平台升级
- **PR #138 `ce39c39`**（**docs(adr-006): caveat removal · Ubuntu validated · v0.1 GA 双平台解锁**）· 本机 Kimi（文档专职 · 用户实际执行 commit ce6bc11）· ADR-006 标题 + 状态 + 日期 + 决策者全升级 · 新增 § Ubuntu Phase B 验证摘要（X11 108ms · Wayland 107ms · 30 cold boot 0 fail · IME fcitx5 PASS · 5min 稳定）· CLAUDE.md A 栏 #19 双平台数据同步 · PROGRESS.md 三处同步 · SPIKE-01/02 spec frontmatter `blocked → done` · session-history/README.md 加 session 19 占位 · 0 代码改动
- **PR #137 `352dda5`**（**spike(spike-01-02): Phase B Ubuntu Wayland+X11 验证**）· kimi-ubuntu24 · 30 cold boot 测试 · X11 / Wayland 双 backend · IME fcitx5 中文 conditional PASS · `pnpm tauri build` 成功 · `docs/spikes/SPIKE-01-report.md` + `SPIKE-02-report.md` Phase B 段落 · `docs/runtime-evidence/spike-01/` + `spike-02/` 截图归档 · ADR-006 caveat 解除条件达成 · 开通 PR #138 ADR 升级
- **PR #133 `dc0412d`**（**docs(progress): MVP-11 5/5 全 done + 本地分支 cleanup**）· Claude Code 主 agent · PROGRESS.md 同步 PR #129 + Phase 2 ✅ + 本地分支 18 origin/ → 1 cleanup 总结
- **PR #129 `a18b7a5`**（**MVP-11 Phase 2 Title bar Overlay ✅ done**）· Codex CLI 实施（commit e09a09d author=Leafile Lune · trailer 标 Codex）· 主 agent rebase + 解冲突（styles.css auto-merge · spec MVP-11 手动 resolve · 修 PR # 引用错 #129→#130）· `crates/app/src/lib.rs` `configure_title_bar` 仅 macOS 编译 · `tauri.conf.json` `titleBarStyle: "Overlay"` + `hiddenTitle: true` + `trafficLightPosition` · `web/src/App.tsx` `data-tauri-drag-region` + `startDragging()` · `.title-bar-drag` 仅 `.platform-macos` 显示 · sidebar 延伸到 title bar · macOS runtime pass · Ubuntu GUI 未测（R-PHASE-2.linux 技术债 · kimi-ubuntu24 SPIKE-01+02 Phase B 一并验证）· 02-title-bar-overlay.png 截图
- **PR #131 `74cb969`**（**MVP-11 Phase 1 §A.4 webview 行为禁用 30s 录屏 + Phase 1 升级 ✅ done**）· Claude Code 主 agent · `docs/runtime-evidence/mvp-11/07-webview-disabled-behaviors.mov` 7.9 MB / 30s / 2800x1800 retina · `pnpm tauri:build --debug --no-bundle` production binary（frontend prod mode `import.meta.env.PROD=true` disable logic 生效）· `screencapture -V 30 -x -R 100,100,1400,900` + `osascript keystroke` 自动化模拟 Cmd+R / Cmd+- / Cmd+= / Cmd+0 / AXShowMenu · Phase 1 表格 🟡 → ✅ done · R-PHASE-1 升级 "Phase 1 全部 manual capture done"（仅 A.5 Linux 待环境）
- **PR #130 `64d2d9a`**（**MVP-11 Phase 4 D.7 视觉对比 + Phase 1 A.2 截图 + theme race 修复**）· Claude Code 主 agent · 三层 fix：(a) `#root` 改 `opacity: var(--bg-opacity)` whole-tree 半透明（MUX0 风格 · 替代 background rgba · 避免 fully-opaque children 遮蔽）· dark theme 误用 `--unfocused-opacity` 修正为 `--bg-opacity`；(b) `web/src/stores/settings.ts` `applyCssVars` 加 `dataset.theme` apply · `web/src/stores/theme.tsx` 删 `setAttribute("data-theme")` · 解决 ThemeProvider IPC fail fallback "auto" 覆盖 dark theme 的 race condition；(c) 自动化截图工具链 · `01-vibrancy-macos.png`（A.2）+ `05a/05b/05c-opacity-{0.5,0.85,1.0}.png`（D.7 三档 dark theme）· Phase 4 表格 🟡 → ✅ done · Phase 1 仍部分 done（A.2 done · A.4 待 #131）
- **PR #128 `4abf1b3`**（**MVP-11 Phase 4 §D.6 重启持久化自动化测试 + spec 升级 D.6 done**）· Claude Code 主 agent · `crates/core/src/app_settings.rs` 新增 `settings_persist_across_pool_reopen` 测试（10 个 app_settings test 中第 10）· 真实 sqlite 文件 · pool drop 模拟 app 重启 · 9 字段（theme / font_size / bg_opacity / bg_blur / window_padding_x/y / cursor_style / cursor_blink / unfocused_pane_opacity）reopen 还原验证 · 浮点 EPSILON 比较 · 替代原 D.6 manual QA · spec §D.6 acceptance `[ ]` → `[x]` · Phase 4 表格状态 D.1-D.5 → D.1-D.6 done · R-PHASE-4 仅剩 D.7 Opacity 3 档对比截图待 Arbiter 本地 10 min 或推 v0.2
- **PR #127 `6a3f0f4`**（**MVP-11 Phase 4 · Appearance 字段对标 MUX0 + cursor/bg_blur 接通**）· OpenCode（D.1-D.5 round 1 commit 0a32599）+ Claude Code 主 agent（round 2 fix-up commit 05199d1）· `AppearanceGroup.tsx` 扩 7 字段（Background Opacity / Blur / Padding X/Y / Cursor Style / Cursor Blink + Unfocused Pane Opacity）· `app_settings` KV 扩 7 keys + `AppSettings` / `SettingsUpdateRequest` ts-rs binding · `settings_get` / `settings_update` IPC + `settings_changed` event · G.3 H2 regression proof（`bg_opacity → background_opacity` 双链路 cargo build + tsc FAIL）· **round 2 接 3 处空心字段**：(a) bg_blur CSS var 无人消费 → `#root` 加 `backdrop-filter: blur(var(--bg-blur))` 与 hudWindow Vibrancy 叠加 · (b) cursor_style/cursor_blink 写 CSS 但 xterm 不读 CSS → `TerminalPane.tsx` 用 settings 调 `term.options` + `createEffect` 同步 · (c) spec ✅ done → 🟡 部分 done + R-PHASE-4 技术债（D.6 重启持久化 manual QA + D.7 Opacity 3 档对比截图待 Arbiter 30 min 或推 v0.2 · v0.2 可改 `setEffects()` 切 macOS material 实现真"调整 blur 半径"）
- **PR #126 `e10b4dc`**（**MVP-11 Phase 3 round 2 · 修 PR #124 review 5 处 UX bug**）· Claude Code 主 agent · `term.reset()` 替代 SIGINT（HIGH-1 clear_terminal 行为修正 · 之前会中断 shell · 现在仅清屏）· `Position::Logical` 替代 `Position::Physical`（HIGH-2 HiDPI 错位 · 前端 `e.clientX/Y` 是 CSS 逻辑像素）· close_other_tabs / close_tabs_to_right 改 await 串行（WARN-2 fire-and-forget 引发 dialog 竞态）· copy/paste/select_all 加前端 case + PaneApi 扩展 5 method · App.tsx 移除 ⌘, keydown 让 Menu Accelerator 单源（INFO-2 双触发修复）
- **PR #125 `304e040`**（**MVP-11 spec round 3 · macos-private-api 双启用 + R-PHASE-1 技术债**）· Claude Code 主 agent · 自检发现 round 2 把 macOS 私有 API 错列为 "非 Cargo feature" · cargo build 实测验证 Tauri 2 强制 `tauri.conf.json macOSPrivateApi: true` + `Cargo.toml features = ["macos-private-api"]` 双启用 · 否则 build error `dependency features ... does not match the allowlist defined under tauri.conf.json` · spec 3 处 wording 修正 · R-PHASE-1 / R-PHASE-1.linux 技术债条目 · 留 manual capture 给 Arbiter
- **PR #124 `ae68c73`**（**MVP-11 Phase 3 · Native Context Menu + 快捷键 done**）· Kimi 1（远程） · `crates/app/src/menu.rs` 256 行 · macOS NSMenu / Linux GTK fallback · 5 项 tab context（close / close_others / close_right / rename / duplicate）+ 4 项 terminal context（copy / paste / clear / select_all）+ 5 项 app accelerator（Cmd+W / T / D / Shift+D / ,）· `menu_show_tab` / `menu_show_terminal` / `menu_register_shortcuts` IPC · 主 agent 修 3 处编译错（`ContextMenu` trait import / `popup_at(window)` owned 非 borrow / `app.set_menu` return Result 类型）+ cargo fmt（Kimi 漏跑）
- **PR #123 `ab44bbe`**（**MVP-11 Phase 1 · macOS Vibrancy + 透明窗口 + 禁 webview 行为 done**）· OpenCode · `tauri.conf.json` `macOSPrivateApi: true` + `windowEffects.effects: ["hudWindow"]` + `transparent: true` + `trafficLightPosition: {x:20, y:20}` · `Cargo.toml` `tauri features = ["macos-private-api"]` · CSS body 透明 + #root rgba(.85/.75) light/dark + `.platform-linux` fallback rgba(.98) · webview production 禁 contextmenu / Cmd+R / Cmd+- 缩放 · 🟡 部分 done · R-PHASE-1 截图 + 30s 录屏待 Arbiter manual capture
- **PR #122 `ab6f64a`**（**MVP-11 Phase 5 · 字体对齐 HIG（系统字体 · 不 bundle）done**）· Kimi 2 · `web/src/styles/typography.css` 新建 · `--font-ui` -apple-system → SF Pro Display → system-ui → Inter / Segoe UI fallback · `--font-mono` ui-monospace → SF Mono → JetBrains Mono → Menlo / Consolas fallback · `web/src/stores/settings.ts` 加 fontFamily 动态覆盖 `--font-mono` · 不 bundle 字体（HIG 策略）
- **PR #121 `69f0988`**（**CI 关闭 main push trigger · GitHub Actions billing 修复**）· Claude Code 主 agent · `.github/workflows/ci.yml` / `secret-scan.yml` / `task-spec-validator.yml` 去掉 `push: branches: [main]` 触发 · 仅保留 `workflow_dispatch:` · 因 GitHub Actions billing 失败导致 main push 后所有 job 2 秒 fail（误导未来 agent）· "合入后 CI 验证" 规则改为本地 7 gate（cargo clippy/fmt/test + pnpm typecheck/lint + spec validator + whitespace）· billing 恢复后通过 workflow_dispatch 批量回扫
- **PR #120 `df6f99f`**（**MVP-10 Phase B Sentry Spike + ADR-015 proposed**）· Codex CLI · `docs/spikes/code/SPIKE-MVP-10-telemetry/` Sentry SDK 0.47 集成 spike + cargo-bloat release example 实测 · `docs/adr/ADR-015-telemetry-stack-sentry.md` proposed · 决策：Sentry crate 0.47 + sanitized payload + before_send 移除 contexts.trace span_id/trace_id（R-trace 风险）+ default_integrations = false · §H.1.1 4 步验证全过 · round 2 spike code 归档 + ADR Step 3 number 澄清（1.8 MiB 是示例 · sentry 增量 ~500-700 KiB）· **待 Arbiter approve 解锁 Phase B SDK 编码**
- **PR #119 `e580a43`**（**MVP-11 Native Feel Quality spec 草稿 + session 19 入口同步**）· Claude Code 主 agent · `docs/tasks/MVP-11-native-feel-quality.md` 草拟 · 5 phase（Vibrancy / Title bar / Menu / Appearance / Typography）· §A-E acceptance + §H 决策（webview transparent · macos-private-api · `windowEffects.effects` 用 vibrancy 不用 blur · trafficLightPosition · CSS rgba 双主题）· R31 / R-PHASE-E / R-PHASE-1 风险条目 · self-review 找出 8 处问题 commit b5a14ea 修
- **PR #118 `7fc20e1`**（**MVP-09 Phase B · Status 面板 Stage/Unstage + CommitBar UI + 错误对话框 done**）· Kimi 1（远程） · Status 面板加 Stage / Unstage / StageAll / UnstageAll 按钮 · CommitBar 新建组件（commit message + author 显示 + Commit 按钮）· 错误对话框 IdentityMissing / DetachedHead / PreCommitHook · round 2 修 detachedHead 死循环（retry 引发 dialog 重开 · 改单按钮 OK + toast）· StageAll/UnstageAll missing revert 修复 · console.log 清理 · IdentityDialog email 校验
- **PR #117 `124ed85`**（**MVP-08 Phase E fix-up · DevTools 量化 + Phase D PNG 重命名 + spec done**）· OpenCode · 第 5 张 fs watch 实时刷新录屏补齐 · A.2 / A.6 / F.3 DevTools P99 测量 procedure · Phase D 7 PNG 按 ADR-011 R3 语义化前缀重命名 · 🟡 partial done · OpenCode hallucinate 4 数字（A.2 9.4ms / A.6 12ms / F.3 54ms / F.6 280ms · metrics 文件实际仅含 Criterion data + manual procedure）· 主 agent round 2 commit 6d04fb8 降为 partial done + R-PHASE-E 技术债（3 DevTools P99 + 4 静态 UI 截图待 Arbiter manual capture 30-60min · 或推 v0.2）

session 19 特色：

- **史上最高产 29 PR merge**（PR #117-#145 · 单 session）· **MVP-11 + MVP-05 Phase B/C-scaffolding + ADR-006 Ubuntu validated + branch protection 机械化** 一波收官
- **MVP-11 Native Feel Quality 完整收官 🎉**（对标 MUX0 治"web 套壳"观感）· 5/5 Phase ✅ 全 done（Phase 1 Vibrancy / Phase 2 Title bar Overlay / Phase 3 Native Menu / Phase 4 Appearance / Phase 5 Typography）· **零技术债交付**（A.5 + R-PHASE-2.linux Linux 部分由 PR #139 kimi-ubuntu24 round 3 真实 capture 补完）· 主 agent 自动化工具链 capture 全部 manual evidence
- **MVP-05 Phase B 完整落地 + Phase C scaffolding 完成**（PR #141-#144 · 4 PR 一波）· Phase B 后端 + IPC + 30 单元测试覆盖 §H.2 layout / §H.3.1 atomicity / §C smart layout · Phase C 3 SolidJS 组件 + 快捷键 hook 独立编译通过 · Phase C 完整版（Terminal.tsx 集成 + 拖拽 splitter + Smart Layouts UI + §F 性能 + Phase D runtime 证据）留 PR #146+
- **ADR-006 Ubuntu validated · v0.1 GA 双平台解锁**（PR #137-#138）· kimi-ubuntu24 远程独立电脑实测 · 30 cold boot 0 fail · X11 108ms / Wayland 107ms · v0.1 GA 发布策略 macOS-first → 双平台
- **Branch protection 机械化**（PR #145）· `.githooks/pre-push` + `package.json prepare` + `core.hooksPath` 自动配置 · 每台机器 clone + pnpm install 即激活 · 防止任何 agent（含主 agent / Codex / OpenCode / Kimi-ubuntu24）误推 main · `SKIP_BRANCH_PROTECT=1` Arbiter override · 4 场景实测全过 · CLAUDE.md L131 文档化
- **Sub-agent 后台并发协作模式成型**（PR #140 / PR #141）· 用 isolation: worktree + run_in_background 跑 Claude Code sub-agent · 主 agent 监听通知后 push + PR + merge · 替代 OpenCode/Codex（已离开 · 团队收缩 3 人）
- **本地分支全 cleanup**（session 19 收尾 · 18 origin/ branches → 1 仅 main · 4 个 stale local branches 删除 · 多次 worktree remove · PR #129 conflict 解决）
- **跨 agent fix-up pattern 成型**：主 agent 在外部 agent PR 后立即接 round 2/3 修编译错 / 格式化 / hallucinate / UX bug / 空心字段 / 视觉语义错 / B4 真实捕获 · 8 PR 实证
- **主 agent macOS GUI 自动化突破**：用 `screencapture -R/-V` + `osascript AXRaise/keystroke` + Read 工具视觉验证 + sqlite3 直改 KV + pkill+restart dev tree 工具链 · 替代"必须 Arbiter 本地补"的常规假设 · 解决 R-PHASE-1 + R-PHASE-4 共 5 项 manual capture
- **Tauri 2 dual-enablement 实测发现**：`tauri.conf.json macOSPrivateApi: true` 和 `Cargo.toml features = ["macos-private-api"]` 必须双启用
- **CI billing failure 应对**：workflow_dispatch only（PR #121）· 转向本地 7 gate
- **ADR-015 Sentry Spike**：proposed 待 Arbiter approve · 解锁 MVP-10 Phase B
- **Author 归属防御性 unset**：主 agent 起手 `git config --local user.name "Claude Code"` 覆盖 · 全部 17 PR（session 19 主 agent 实施部分）无跨 agent author 错归
- **v2-D.1 PR body trailer 100% 合规**（session 19 全部 17 主 agent PR 三行 trailer 齐 · 无缺失）
- **15 ADR**：ADR-015 proposed · 14 历史全 accepted（含 ADR-006 升级到 Ubuntu validated）

> **滚动窗口前**：session 18 及更早（PR #1-#116）的完整摘要请查 `git log --all --oneline | grep PR` · 或 `docs/session-history/` 里的归档文件。本 PROGRESS 每 session 末按 M-2 规则整理（保留最近 2 session · 当前窗口 session 19 · session 17/18 已归档至 `docs/session-history/`）。
