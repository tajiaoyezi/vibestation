...
## v2.1 增补：§5.4 Git 操作模型（2026-04-18 补建）

> **背景**：v2 主体的 §5 数据模型只到 §5.3（窗口/Tab/Pane）后直接进 §6（IPC 接口设计）。v0.2 的 push/pull/fetch（MVP-11）+ branch CRUD（MVP-13）+ commit rail graph（MVP-12）和 v0.3 的 rebase/merge/cherry-pick（MVP-16）缺少专属规格章节，曾导致这 4 个 MVP spec 的 `plan_ref` 引用悬空（指向不存在的 `§5.4`）。
> **本节作为 v2.1 增补**，按 `CLAUDE.md` 禁区"允许新增 v2.x 增补子节"的约定追加在文档末尾，提供这 4 个 MVP 的战略依据。**不重排原 §5/§6 章节结构**。
> **覆盖 MVP**：MVP-11（push/pull/fetch）· MVP-12（commit rail graph）· MVP-13（branch CRUD）· MVP-16（rebase/merge/cherry-pick）。

### §5.4.1 操作分层

Vibestation 的 Git 操作分 4 层，**自顶向下严格约束**：

| 层 | 范围 | 实现位置 | MVP 引入版本 |
|----|------|---------|-------------|
| **L1 · 只读视图** | log / status / diff / blame | `core::git::read` | v0.1 MVP-07/08 |
| **L2 · 本地写** | stage / unstage / commit / amend | `core::git::write_local` | v0.1 MVP-09 |
| **L3 · 远端同步** | push / pull / fetch / clone | `core::git::remote` | v0.2 MVP-11 |
| **L4 · 历史改写** | rebase / merge / cherry-pick / reset | `core::git::history` | v0.3 MVP-16 |

**严格分层原则**：L4 不直接调 L1，必须经 L2/L3 中转；L3 不直接动 working tree（先 fetch 到 `FETCH_HEAD`，再走 L2/L4 合并）。

### §5.4.2 IPC 命令补充（基于 §6 命名原则）

新增以下命令（IPC 命名遵循 §6.1 `<domain>.<verb>` 约定）：

```
# L3 远端（MVP-11）
git.remote.list                  # 列 remotes（origin / upstream / ...）
git.remote.add { name, url }
git.remote.remove { name }
git.fetch { remote, prune }      # 异步，进度通过 git.fetch.progress 事件流回
git.pull { remote, branch, strategy }   # strategy: ff-only | rebase | merge
git.push { remote, branch, force_with_lease }   # 默认 disable force

# L2/L3 分支（MVP-13）
git.branch.list { include_remote }
git.branch.create { name, base }
git.branch.checkout { name, create_if_missing }
git.branch.delete { name, force }
git.branch.rename { old, new }

# L4 历史改写（MVP-16）
git.rebase.start { onto, from }       # 进入 rebase 状态机
git.rebase.continue
git.rebase.abort
git.merge { branch, no_ff }
git.cherry_pick { commit_sha }
git.reset { mode, commit }            # mode: soft | mixed | hard
```

**所有 L3/L4 命令必须返回 `OperationId`**，前端通过 `git.<op>.progress` / `git.<op>.complete` / `git.<op>.error` 事件订阅状态变化。**禁止同步阻塞**（防 UI 卡死）。

### §5.4.3 凭据管理（MVP-11 强依赖）

**默认走系统 keyring**：

| 平台 | 后端 | crate |
|------|------|-------|
| macOS | Keychain | `keyring` 或 `security-framework` |
| Linux | Secret Service / libsecret | `keyring` |
| Windows（v0.4） | Credential Manager | `keyring` |

**凭据 fallback 链（按顺序尝试）**：

1. SSH agent（`SSH_AUTH_SOCK` 探测）
2. Git credential helper（已配置 `credential.helper = osxkeychain` / `libsecret` / etc.）
3. 系统 keyring（key = `vibestation:remote:<remote-url>`）
4. 弹窗提示用户输入（一次性，可选保存到 keyring）

**禁区**：
- ❌ **禁止把 token / 密码写入 redb**（用户敏感数据隔离）
- ❌ **禁止在日志 / telemetry 里出现 remote URL 含凭据的形式**（必须 `git_url::strip_credentials` 后输出）

### §5.4.4 Commit Rail Graph 数据模型（MVP-12）

Rail graph 是"自绘"（`CLAUDE.md` 决策表 #7 Diff 自建延伸到 graph 自绘），**不依赖 git CLI 的 `--graph` 输出**：

```rust
struct CommitRailNode {
    sha: String,
    parents: Vec<String>,          // 多 parent = merge commit
    rail_index: u8,                // 0..N · 该 commit 在哪条 rail
    rail_color: u8,                // 0..palette_size · 决定 rail 颜色
    branch_tags: Vec<BranchTag>,   // 该 commit 有哪些 branch HEAD 指向
}

enum RailEvent {
    Continue,      // 同 rail 延续
    Branch,        // 当前 commit 派生新 rail（fork 点）
    Merge,         // 多 rail 汇入当前 commit（merge 点）
    End,           // 当前 rail 终结（被 merge 后无后续 commit）
}
```

**算法约束**：
- Rail 分配走"贪心 + 复用空闲 rail"策略，**不等价于 `git log --graph` 输出**（git CLI 的算法在 octopus merge 上分歧大，自绘允许更好的视觉规则）
- 大仓库（10 万 commit+）必须**只渲染可见区段 + 虚拟滚动**，禁止一次性算全图（会 OOM）
- Rail color 调色板从 `design/directions/1-calm-studio.html` `--rail-palette-*` 变量取，禁止硬编码

### §5.4.5 Rebase / Merge / Cherry-pick 状态机（MVP-16）

L4 操作进入"长事务"状态机，通过 `RepoOperationState` 持久化（崩溃恢复时可继续）：
...
**关键约束**：
- 任一时刻**仓库只能有一个 L4 操作 in_progress**（`git2` 本身就有这个约束 · 双开会被拒）
- 进入 `awaiting_user` 时，UI 必须显示冲突文件列表 + "继续 / 中止" 按钮
- **崩溃恢复**：启动时检测 `.git/rebase-merge` / `.git/rebase-apply` / `.git/MERGE_HEAD` 等标记，主动提示用户"上次操作未完成，继续 / 中止？"
- **Force push 防误操作**：MVP-11 默认 `--force-with-lease`，**永不**使用裸 `--force`

### §5.4.6 错误分类与处理

| 错误类 | 来源 | 用户可恢复 | UI 行为 |
|--------|------|-----------|---------|
| `NetworkError` | fetch / push / pull | ✅ 重试 | toast + "重试"按钮 |
| `AuthError` | credential 失败 | ✅ 重新输入 | 弹凭据对话框 |
| `NonFastForward` | push 被拒 | ✅ pull → 重试 | 提示 + "拉取后重推"按钮 |
| `MergeConflict` | merge / rebase / cherry-pick | ✅ 解决冲突 | 进入 `awaiting_user` 状态 |
| `Detached` | checkout commit-sha 后 | ✅ 创建分支或回到分支 | 顶部 banner 提示 |
| `LockedRef` | 并发操作冲突 | ⚠️ 等待解锁 | 队列化重试（最多 3 次） |
| `CorruptRepo` | `.git` 损坏 | ❌ 用户介入 | 弹"打开外部终端修复"对话 |

**所有错误必须**：
1. 包含原始 `git2::Error` 的 `code` + `class`（不能只透传 message）
2. 含一句话用户文案（中文）+ "查看技术详情"折叠面板
3. 关键错误（CorruptRepo / LockedRef 长时间）写入 `docs/spike-artifacts/<MVP-NN>/incident-*.log`（非生产用，本地诊断）

### §5.4.7 与现有章节的映射

- 数据模型：本节扩展 §5.1（核心实体）的 `Workspace.git_status` 字段（v0.2 起需新增 `pending_l4_op: Option<RepoOperationState>`）
- 持久化：`RepoOperationState` 写入 redb / rusqlite（取决于 SPIKE-04 决策），key = `workspace:<id>:git:pending-op`
- IPC：本节 §5.4.2 命令清单是 §6 IPC 接口设计的 v0.2/v0.3 增量
- 风险：与 §9 的 R3（git2 大仓性能）、R10（git2 凭据 helper 兼容性）、R15（rebase 中断恢复）相关
- MVP 范围：本节是 §10.1 MVP B 折中 "v0.2 推到后续"那部分功能的展开

### §5.4.8 v2.1 增补 changelog

| 日期 | 修改 | 触发 |
|------|------|------|
| 2026-04-18 | 首次建立 §5.4 Git 操作模型章节 | Codex 评审发现 MVP-11/12/13/16 的 `plan_ref §5.4` 悬空 · 修复战略空缺 |
