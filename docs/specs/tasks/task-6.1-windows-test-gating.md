...
- [ ] **AC1** (PRD §Decisions Log D5 / §Success Metrics 主要指标): Windows 上 `cargo test --workspace` 0 编译错误、0 `panicked`，现存 Unix-only 测试显示 `ignored`（每条带明确根因注释）。
- [ ] **AC2** (PRD §Success Metrics 反指标): mac/Linux 上被门控测试行为零回归——`cargo test --workspace` 仍 100% 绿，原 `#[cfg_attr(target_os = "linux", ignore)]` epoll-timing 标记保留，门控只对 Windows 新增生效。
- [ ] **AC3** (PRD §Decisions Log D5): `git_ops_integration.rs` 的 `use std::os::unix::fs::PermissionsExt` + `create_pre_commit_hook`（`chmod 0o755` + `#!/bin/sh`）走 `#[cfg(unix)]`，Windows 上不编译该 Unix-only 路径；非 hook commit 测试保持跨平台。
- [ ] **AC4** (PRD §User Flow 异常流 / §Technical Risks R4): `pty_pool_bench.rs` 的 `/tmp` 硬编码改为 `std::env::temp_dir()`、`HOME` 回落改为 `USERPROFILE`（Windows）/`HOME`（Unix）兜底到 `temp_dir()`，Windows 上不再 `panicked` on missing `/tmp`。
- [ ] **AC5** (本 task 新增): 至少补一条 `#[cfg(target_os = "windows")]` 正向测试（如 `temp_dir()` / `USERPROFILE` 解析），让 Windows 有真实正向覆盖而非全 skip。

## 7. SDD / BDD / TDD Traceability

| Acceptance Criterion | BDD Scenario | TDD Test | Integration / E2E Test | Verification | Status |
|---|---|---|---|---|---|
| AC1: Windows test 0 panicked · Unix-only ignored | SCEN-6.1.1 | TEST-6.1.1 `test_6_1_1_windows_temp_dir_and_userprofile_resolve` + Windows `cargo test --workspace` 输出无 panicked | N/A（cargo test 即集成）| `cargo test --workspace`（Windows · 断言 0 panicked + ignored>0）| Not Started |
| AC2: mac/Linux 零回归 | SCEN-6.1.2 | TEST-6.1.2 既有 Unix 测试集（`zsh_01_*` / `stdout_is_persisted_*` 等）执行通过 | N/A | `cargo test --workspace`（macOS + Ubuntu · 与基线比对）| Not Started |
| AC3: hook 测试 `#[cfg(unix)]` 门控 | SCEN-6.1.3 | TEST-6.1.3 `create_pre_commit_hook` + 依赖测试 Unix-only 编译 | N/A | `cargo check --workspace`（Windows · 不编译 unix path）+ `cargo test`（Unix · hook 测试仍跑）| Not Started |
| AC4: `/tmp`→`temp_dir()` · `HOME`→`USERPROFILE` 兜底 | SCEN-6.1.4 | TEST-6.1.4 `pty_pool_bench` `user_home()` / `bench_cwd()` 跨平台解析 | N/A | `cargo test --test pty_pool_bench -- --ignored`（Windows · 不 panic on `/tmp`）| Not Started |
| AC5: ≥1 Windows 专属正向测试 | SCEN-6.1.5 | TEST-6.1.5 `test_6_1_1_windows_temp_dir_and_userprofile_resolve` | N/A | `cargo test --workspace`（Windows · 该测试 passed）| Not Started |
