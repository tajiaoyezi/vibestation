...
export const closeSession = (sessionId: string): Promise<void> => invoke("close_session", { sessionId });
export const closeAllSessions = (): Promise<void> => invoke("close_all_sessions");
export const sessionSnapshot = (sessionId: string): Promise<SessionSummary> => invoke("session_snapshot", { sessionId });
export const managerSnapshot = (): Promise<SessionSummary[]> => invoke("manager_snapshot");
export const writeArtifact = (request: ArtifactWriteRequest): Promise<void> => invoke("write_artifact", { request });
export const readArtifact = (request: ArtifactReadRequest): Promise<string> => invoke("read_artifact", { request });
