...
fn drain_session(
    ...
    state
        .drain_session(
            &request.session_id,
            request.max_chunks.unwrap_or(32),
...
fn runtime_config() -> RuntimeConfig {
    RuntimeConfig {
        scenario: std::env::var("SPIKE05_SCENARIO").ok(),
        output_dir: std::env::var("SPIKE05_OUTPUT_DIR").ok(),
        report_dir: std::env::var("SPIKE05_REPORT_DIR").ok(),
        run_label: std::env::var("SPIKE05_RUN_LABEL").ok(),
        close_on_complete: std::env::var("SPIKE05_CLOSE_ON_COMPLETE")
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|_app| {
            eprintln!("[SPIKE-05] app pid={} scenario={:?}", std::process::id(), std::env::var("SPIKE05_SCENARIO").ok());
            Ok(())
        })
