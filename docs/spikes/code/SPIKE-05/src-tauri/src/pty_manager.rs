...
pub struct PtyManager {
    ...
    control_tx: mpsc::Sender<ReaderCommand>,
...
impl PtyManager {
    pub fn new() -> Self {
        ...
        thread::spawn(move || reader_loop(control_rx, reader_alive_thread));
    ...
    pub fn close_session(&self, session_id: &str) -> io::Result<()> {
        ...
        let _ = self.control_tx.send(ReaderCommand::Unregister {
            token: session.token,
            fd: session.fd,
        });
        session.kill();
    ...
    pub fn sample_process_stats(&self) -> io::Result<ProcessStats> {
        ...
        Ok(ProcessStats {
            ...
            reader_thread_alive: self.reader_alive.load(Ordering::Relaxed),
        })
    }
...
impl Drop for PtyManager {
    fn drop(&mut self) {
        let _ = self.control_tx.send(ReaderCommand::Shutdown);
...
fn reader_loop(control_rx: mpsc::Receiver<ReaderCommand>, reader_alive: Arc<AtomicBool>) {
    ...
    while reader_alive.load(Ordering::Relaxed) {
            ...
            match command {
                ReaderCommand::Register { token, fd, session } => {
                    ...
                    if let Err(error) = poll.registry().register(&mut source, Token(token), Interest::READABLE) {
                        eprintln!("[SPIKE-05] register({token}) failed: {error}");
                        continue;
                    }
        ...
        if let Err(error) = poll.poll(&mut events, Some(Duration::from_millis(50))) {
            eprintln!("[SPIKE-05] poll failed: {error}");
            continue;
        }
        ...
        for event in events.iter() {
            ...
            if event.is_readable() {
                read_session_fd(session, DropPolicy::DropOldest);
            }
        }
    }
}

fn read_session_fd(session: &Arc<SessionState>, policy: DropPolicy) {
    ...
    loop {
        ...
        if read > 0 {
            session.enqueue_bytes(&buffer[..read as usize], policy);
            continue;
        }
        ...
        match error.raw_os_error() {
            ...
            _ => {
                eprintln!("[SPIKE-05] read fd {} failed: {error}", session.fd);
                break;
            }
        }
    }
