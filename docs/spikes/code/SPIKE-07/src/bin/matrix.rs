//! Phase C · §F 测试矩阵 harness（主 agent · decision-grade）。
//!
//! 对 36 条 SPIKE-06 corpus 逐条：解码 → 路由 parser → catch_unwind parse →
//! `assertions::assess` 按 §F 逐断言判定 → 汇总场景/CLI/整体正确率 + §E.11 基线对比。
//!
//! "正确率"定义：单样本 `!panicked && all_passed` → PASS；场景正确率 =
//! PASS/该场景样本数；整体加权 = PASS/36（每样本等权 · spec §E.3）。
//!
//! 输出：markdown 全量矩阵 → stdout（捕获到 `docs/spikes/raw/SPIKE-07/`）。
//! 设 env `SPIKE07_JSON=<path>` 时另写机器可读 JSON（§E.7 无 hardcode 路径）。
//! 复现：`cargo run --bin matrix`（在 `docs/spikes/code/SPIKE-07/`）。

use spike_07_parser::assertions::{
...
use spike_07_parser::ir::{CliEvent, ParseInput};
...
fn main() {
    ...
    for f in &corpus {
        ...
        let assessment = assess(&f.scenario, &events, &f.decoded.output);
        let parser_emits_error = events.iter().any(|e| matches!(e, CliEvent::Error { .. }));
        let scenario_truth_error = scenario_is_error(&f.scenario);
    ...
    let overall_accuracy = pct(overall_pass, total);
    ...
    // §E.11 基线对比：error-detection（parser Error 事件 vs 关键字/行首启发式）
    let corpus_inputs: Vec<(&str, &str, Option<i64>, bool, bool)> = rows
        .iter()
        .zip(corpus.iter())
        .map(|(r, f)| {
            (
                r.scenario.as_str(),
                f.decoded.output.as_str(),
                f.decoded.exit_code,
    ...
    let parser_correct = corpus_inputs
        .iter()
        .filter(|(_, _, _, p, truth)| p == truth)
        .count();
    let kw_correct = corpus_inputs
        .iter()
        .filter(|(_, out, ec, _, truth)| &baseline_keyword_is_error(out, *ec) == truth)
        .count();
    let lp_correct = corpus_inputs
        .iter()
        .filter(|(_, out, _, _, truth)| &baseline_lineprefix_is_error(out) == truth)
        .count();
    ...
    if let Ok(path) = std::env::var("SPIKE07_JSON") {
...
fn print_markdown(r: &MatrixReport) {
    ...
    println!("| --- | --- | --- |");
    println!(
        "| Parser（Error 事件） | {:.0}% | 结构化解析 |",
        r.baseline.parser_errdetect_accuracy
    );
    println!(
        "| 基线 A 关键字扫描 | {:.0}% | exit≠0 / error/unauthorized/failed 子串 |",
        r.baseline.keyword_baseline_accuracy
    );
    println!(
        "| 基线 B 行首启发式 | {:.0}% | `^Error:` / `^WARNING:` |",
    ...
    println!(
        "\n> Parser − 最优基线 = **{:+.0}pp**（§E.11 要求 parser 显著优于基线 +20pp 才值复杂度）",
        delta
    );

    println!("\n## 逐样本矩阵（36 条 · 每条 §F 断言）\n");
    println!("| # | CLI | 场景 | take | events | unrec | panic | 断言 (pass/total) | 样本 |");
    println!("| --- | --- | --- | --- | --- | --- | --- | --- | --- |");
