...
- [ ] Dialogs remaining（仅通用 chrome）：Config Import、Create Branch、Merge / Cherry-pick / Force / Dirty tree / Remote Selector 等 Git operation dialogs 的 `Cancel`、`Retry`、`Close <name> dialog` aria-label 与 checklist 内固定 empty/loading 文案。
...
> 2026-06-06 回填：FEAT-02 仍为 `in-progress`。FEAT-02.1-02.3 已完成，FEAT-02.4a（Settings first）、FEAT-02.4b（workspace chrome）与 FEAT-02.4c phase 1（Main/Secondary + common dialogs）已完成；dialog remaining、runtime smoke、ADR accepted 翻转与最终 done gate 仍待执行。
...
- 偏离 spec 的说明：
  - 按 Kimi `READY-WITH-NITS` 建议采用 `split-settings-first`，将 FEAT-02.4 拆为 02.4a Settings、02.4b app chrome、02.4c dialogs/common errors；当前声明 02.4a、02.4b 与 02.4c phase 1 完成。
...
- 后续 follow-up：
  - FEAT-02.4c remaining：Config Import 与 Git operation dialogs 的 common chrome，继续排除业务操作长文案与 backend raw errors。
