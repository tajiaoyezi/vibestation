...
**下次 agent 起点**：Phase C · 复用已落地的 `GitStatusPanel` 与 `diff_compute` / `diff_get_settings` contract，把文件点击真正接到 Diff 主区视图；尤其注意 `DiffRequest.allowLargeFile` + `DiffResponse.truncatedReason/lineCount` 已经为大文件 fallback 预留，不要在前端重复发明另一套协议。
