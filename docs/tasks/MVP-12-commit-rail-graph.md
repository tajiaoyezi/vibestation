...
| Phase | 范围 | 状态 | 预估 | 交付物 |
|---|---|---|---|---|
| Phase A · 数据接线 + 布局骨架 | 消费 MVP-07 commit list + refs，完成 lane 分配最小实现，输出布局快照（不含高级优化） | 🟡 planned | 2d | `RailGraphModel.ts` + 快照 fixture |
| Phase B · Canvas 绘制 + 视觉语义 | commit 节点、连线、merge/fork 节点形态、branch tip 标签（local/remote/tag） | 🟡 planned | 2d | `RailGraphCanvas.tsx` + 视觉回归首版 |
