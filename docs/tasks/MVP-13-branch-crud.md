...
> ⚠️ **2026-05-20 · capture mandate removed**（ADR-023 supersede ADR-011）：本 spec 中所有 **"Phase D 截图 / 录屏 / GUI capture / dirty tree 对话框 / Fuzzy Switcher 全键盘流程截图 / manual QA capture" 类 acceptance 项 / Phase 表行** 已 supersede · 不再阻塞 spec done flip。inline 文字保留作 audit 历史 · 但**功能上 deprecated**。代码侧 acceptance（branch_ops::tests:: 32 + Criterion bench Linux 基线 branch_list_10/list 1.06 ms · `bench-output.txt` 归档）保留为 done gate。
...
| Phase D · 性能量化                             | 性能量化（10 / 100 / 1000 branch fixture）放 `docs/runtime-evidence/mvp-13/bench-output.txt` · 截图（CRUD 三大操作 · dirty tree 对话框 · Fuzzy Switcher）capture 要求 deprecated 2026-05-20（ADR-023） | ✅ done · PR #226 | #226 |
...
- ADR-007 Git 栈混用决策
- `implementation-plan.md` §10.1 v0.2 砍到分支 · §6.2 git*branch*\* IPC · §11 W13
...
export function validateBranchName(name: string): {
  ...
  if (FORBIDDEN_CHARS.test(name))
    return { valid: false, reason: "含非法字符（空格 / 控制字符 / ~^:?*[\\）" };
  for (const p of FORBIDDEN_PATTERNS)
    if (p.test(name))
      return {
        valid: false,
        reason: "非法格式（起始 . / 含 .. / @{ / 结尾 .lock 或 /）",
      };
  return { valid: true };
}
```
