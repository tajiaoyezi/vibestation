...
| Phase | 范围 | 状态 | PR |
|-------|------|------|----|
| Phase A · shiki 集成 + lazy load 基础 | `shiki` v3+ 包引入 · `Highlighter` 单例封装 · theme 预加载（light/dark 两套）· lazy load 核心逻辑（IntersectionObserver + 行级虚拟化）· LRU 缓存（100 文件 / 50MB）· 2-4 个 IPC binding（见 §G） | 🔄 待实施 | — |
