...
> ⚠️ **2026-05-20 · capture mandate removed**（ADR-023 supersede ADR-011）：本 spec 中所有 **"§I.5 Phase D runtime evidence 5+ 截图 / Playwright E2E trace / §I.4 Phase D evidence / §F.4 manual screenshot / §H manual QA capture / macOS+Linux+Windows manual QA" 类 acceptance 项 / Phase 表行** 已 supersede · 不再阻塞 spec done flip。inline 文字保留作 audit 历史 · 但**功能上 deprecated**。代码侧 acceptance（pane_links 28 + parser_bridge 8 + mvp18_contract 18 + 前端 pane-linking 98 vitest + §F.3 6/6 fixture / 性能 DevTools 数字）保留为 done gate。
