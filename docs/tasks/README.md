...
| MVP-18     | AI-Aware Pane 联动                        | **in-progress** | SPIKE-07.5 PASS 解锁 · **Phase A/B/C full done**（#344-#364：migration/DAO/IPC/binding + store/seam→binding + failure wire + Wave 1/2/3 + doc-sync 全部 merged · Phase D evidence 待续）· owner Claude Code                                                                                                                                                                                                                                                       | #344-#364                 |
| MVP-19     | session ↔ commit 绑定                     | **in-progress** | session 32 ready-gate + 05-17 Arbiter waive（#365）· **W1 + W2-A.0/B/C/D + W2-doc merged**（types/DAO/engine + IPC contract+bindings + 前端 client + handler + redaction #366-#373）· **Phase C/D/E-impl 全 merged**（E-playbook #376 · E-backend redaction wire #378 · C GitLog 徽章 #377〔CSS reviewer-fix〕· D 详情/modal #379 · 组合 C+D vitest 442 零回归 + dev-boot clean）· **Phase E finalize（capture）defer Arbiter playbook #376** · owner Claude Code | #313 · #330 · #365-#379   |
| MVP-20     | 一键回滚（session revert）                | **in-progress** | session 32 ready-gate + Arbiter approve flip · **Phase A/C/D 全 merged**（A #385-#388 · C #391/#392/#390 · **D #394** status union 保真〔`RollbackStatusKind`〕+ 全局 crash recovery〔detect + emit + banner · 镜像 MVP-16〕+ abort 边界 · 主 agent 自实施 TDD · self-review v2-D.2 + Arbiter approve）· Phase E（runtime/perf capture）defer Arbiter playbook #390 窗口 · spec 保持 `in-progress`（多 phase）                                                    | #312 · #330 · #385-#394   |
...
| [MVP-20](./MVP-20-ai-one-click-rollback.md)         | **AI 一键回滚（session 级 revert）**（v1.0 vision）                 | **in-progress**（session 31 详化 · session 32 Arbiter approve flip · **Phase A/C/D 全 merged** #385-#394：A revert plan/IPC/binding · C 后端 resume+前端 wire+playbook · **D #394** `RollbackStatusKind` union 保真〔spec §K〕+ 全局 `detect_crash_recovery`+emit+`RollbackRecoveryBanner`〔镜像 MVP-16 · session 维度平行〕+ abort 边界 · 主 agent 自实施 TDD · Phase E capture defer Arbiter #390 · spec 保持 `in-progress`） | v1.0     | 6d   | MVP-19                   |
